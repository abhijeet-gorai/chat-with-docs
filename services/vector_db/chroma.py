...
class ChromaRetriever:
    ...
    def list_collections(self) -> List[str]:
        """
        Lists all collection names in the Chroma database.

        Returns:
            List[str]: A list of collection names.
        """
        return [collection.name for collection in self._vector_db._client.list_collections()]
    ...
    def _get_unique_id(self, document: Document) -> str:
        ...
        """
        content_to_hash = document.page_content
        hash_object = hashlib.sha256(content_to_hash.encode("utf-8"))
        document_id = str(int(hash_object.hexdigest(), 16) % (10**18))
        return document_id
    ...
    def add_documents(self, docs: List[Document], collection_name: Optional[str] = None) -> List[str]:
        ...
        for doc in docs:
            doc.id = self._get_unique_id(doc)
        return self._vector_db.add_documents(docs)

    def delete_documents_by_metadata(
        self, metadata_filter: Dict[str, Union[str, int, float]], collection_name: Optional[str] = None
    ) -> bool:
        """
        ...
        Args:
            ...
            collection_name (Optional[str]): The collection to delete from. Defaults to current collection.
    ...
    def get_relevant_docs(
        ...
        """
        docs_with_scores = self.get_relevant_docs_with_similarity_score(
